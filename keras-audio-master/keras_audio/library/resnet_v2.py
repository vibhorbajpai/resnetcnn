...
class ResNetV2AudioClassifier(object):
        ...
        while True:
            for batchIdx in range(0, num_batches):
                ...
                for i in range(start, end):
                    ...
                    #mg = compute_melgram(audio_path)
                    mg = np.load(audio_path)
                    redmg = mg.astype(np.float32)
                    #nmg = mg[:,2500:5000, np.newaxis]
                    nmg = redmg[:,1500:2000,:]
                    X[i - start, :, :, :] = nmg
    ...
    def fit(self, audio_path_label_pairs, model_dir_path, batch_size=None, epochs=None, test_size=None,
        ...
        with open(os.path.join(currentdir, 'fold1indexlist.txt'), 'rb') as jf:
            fold1 = pickle.load(jf)
        with open(os.path.join(currentdir, 'fold2indexlist.txt'), 'rb') as jf:
            fold2 = pickle.load(jf)
        with open(os.path.join(currentdir, 'fold3indexlist.txt'), 'rb') as jf:
            fold3 = pickle.load(jf)
        with open(os.path.join(currentdir, 'fold4indexlist.txt'), 'rb') as jf:
            fold4 = pickle.load(jf)
        with open(os.path.join(currentdir, 'fold5indexlist.txt'), 'rb') as jf:
            fold5 = pickle.load(jf)
            
        test_index = fold5
        train_index = fold1 + fold2 + fold3 + fold4
